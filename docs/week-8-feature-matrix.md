...
| Question bank | Week 13 | Complete | Answer service/action complete | `ApprovedAnswer` and `QuestionMatch` migrations added | Deterministic matching without AI | Unit/E2E passed | Save, list, match, record use | `/question-bank`, `src/features/questions` | AI-assisted answer drafting waits for Week 14 | Complete locally |
| Application package generation | Week 14 | Complete | Package service/action complete | `Application`, `ApplicationMaterial`, `ApplicationQuestion` migrations added | Mock/OpenAI provider boundary | Unit/E2E passed | Job detail package panel | `/jobs/[id]`, `src/features/applications` | Tracker/export waits for Week 15 | Complete locally |
| Application tracking/export | Week 15 | Complete | Tracker actions/routes complete | Uses Week 14 `Application` records | N/A | Unit/E2E passed | Status forms and CSV downloads | `/applications`, `/api/export` | Phase 1 hardening waits for Week 16 | Complete locally |
| Phase 1 release gate | Week 16 | Complete | CI and security/deployment docs complete | Uses all migrations | Mock AI in CI | Unit/E2E passed | Protected routes and export auth checks | `.github/workflows/ci.yml`, `docs/week-16-release-candidate.md` | Rate limiting/deletion remain launch risks | Complete locally |
